...
void parseParameters(VariantCustom helper, HttpMessage msg){
    ...
    for (def attribute : attributes){
        ...
        def name = data[0]
        def value = URLDecoder.decode(data[1], StandardCharsets.ISO_8859_1)
...
void setParameter(VariantCustom helper, HttpMessage msg, String param, String value, boolean escaped) {
    ...
    for(def parameter : helper.getParamList()){
        ...
        if (parameter.getName() == param){
            header += escaped ? value : URLEncoder.encode(value, StandardCharsets.ISO_8859_1)
        }else{
...
return [
    parseParameters:    { helper, msg -> parseParameters(helper, msg) },
    setParameter:       { helper, msg, param, value, escaped -> setParameter(helper, msg, param, value, escaped) }
]
